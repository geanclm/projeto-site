...
class Parameter:    
    prompt = 'Consulte a tabela do próximo concurso da Loteca. Verifique informações adicionais em {urls} e retorne informações sobre o concurso de forma organizada em uma tabela Markdown com uma previsão de resultado para cada jogo em uma coluna correspondente.',
...
#     name: str ="ProgramacaoLotecaTool"
#     description: str = "Buscar, via API, a programação do próximo concurso da Loteca"
    
#     def _run(self, text: str) -> str:
#         # API do próximo concurso
#         url_loteca_programacao = 'https://servicebus2.caixa.gov.br/portaldeloterias/api/loteca/programacao'

#         # Fazendo a requisição para a API
#         response = requests.get(url_loteca_programacao)

#         # Convertendo a resposta para JSON
#         data_loteca_programacao = response.json()

#         # Extraindo a lista de jogos
#         lista_jogos_programacao = data_loteca_programacao[0]['listaJogos']

#         # Criando o DataFrame a partir da lista de jogos
#         df_loteca_programacao = pd.DataFrame(lista_jogos_programacao)

#         # mudar ordem das colunas do DataFrame
#         # new_column_order = ['nuConcurso', 'nuGolEquipeUm','siglaPaisUm', 'siglaUFUm', 'nomeEquipeUm', 'nomeEquipeDois', 'siglaUFDois','siglaPaisDois', 'nuGolEquipeDois','nuSequencial', 'diaSemana', 'dtJogo']
...
#             'siglaUFUm': 'uf1',
#             'siglaUFDois': 'uf2',
#             # 'siglaPaisUm': 'country1',
#             # 'siglaPaisDois': 'country2'
#         })
...
#         realizacao_api = f'Realização dos jogos de futebol: de {data_loteca_programacao[0]["dataInicioRealizacaoJogos"]} a {data_loteca_programacao[0]["dataFimRealizacaoJogos"]}'
#         tabela_api = df_loteca_programacao.copy()
#         tabela_api_xml = tabela_api.to_xml(index=True, root_name='Tabela', xml_declaration=False)
        
#         return concurso_api, periodo_api, realizacao_api, tabela_api, tabela_api_xml